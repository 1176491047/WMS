...
namespace VskyWMS.Blazor.Menus;

public class VskyWMSMenuContributor : IMenuContributor
{
    private readonly IConfiguration _configuration;

    public VskyWMSMenuContributor(IConfiguration configuration)
    {
    ...
    {
        var l = context.GetLocalizer<VskyWMSResource>();
        ...
        context.Menu.AddItem(
            new ApplicationMenuItem(
                VskyWMSMenus.HostDashboard,
                l["Menu:Dashboard"],
                "~/HostDashboard",
                icon: "fa fa-line-chart",
                order: 2
            ).RequirePermissions(VskyWMSPermissions.Dashboard.Host)
        );

        //TenantDashboard
        context.Menu.AddItem(
            new ApplicationMenuItem(
                VskyWMSMenus.TenantDashboard,
                l["Menu:Dashboard"],
                "~/Dashboard",
                icon: "fa fa-line-chart",
                order: 2
            ).RequirePermissions(VskyWMSPermissions.Dashboard.Tenant)
        );
